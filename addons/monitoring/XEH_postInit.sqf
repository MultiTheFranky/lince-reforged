...
["CBA_settingsInitialized", {
	...
	addMissionEventHandler ["EntityKilled", {
		...
		if (isPlayer _killer) then {
			...
			missionNamespace setVariable [format ["lince_kill_%1", _killerName], (missionNamespace getVariable [format ["lince_kill_%1", _killerName], 0]) + 1];
			[_killerName, missionNamespace getVariable [format ["lince_kill_%1", _killerName], 0]] call FUNC(logKill);
		};
	}];
